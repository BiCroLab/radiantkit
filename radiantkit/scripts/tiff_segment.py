...
from radiantkit import const, path, stat, rkstring
...
Input images that have the specified prefix and suffix are not segmented.""",
    ...
    parser.add_argument(
        ...
        dest="gaussian",
        default=0,
        help="""Apply a Gaussian low pass filter to the image""",
    )
...
def parse_arguments(args: argparse.Namespace) -> argparse.Namespace:
    ...
    args.outprefix = rkstring.add_trailing_dot(args.outprefix)
    args.outsuffix = rkstring.add_leading_dot(args.outsuffix)
...
def select_in_focus_slice(
    ...
    """ Return the slice that has most gradient magnitude (proxy for focus)
    For 2D input: returns back the input image.
    """
    if(len(img.shape) == 3):
        L = channel.ImageLabeled(extract_slice(L.pixels, 0, img.focus_slice_id()))
    return L
...
def select_most_populated_slice(
    ...
    """Return the plane containing most labels. WARNING: This is
    probably not a good option. Consider returning the slice with most
    pixels set as nuclei instead.
    """
    ...
    for slice_id in range(img.shape[0]):
        object_counts.append(
            ski.measure.label(extract_slice(L.pixels, 0, slice_id)).max()
        )
    max_objects: int = max(object_counts)
    if 1 == object_counts.count(max_objects):
        return channel.ImageLabeled(
            extract_slice(L.pixels, 0, object_counts.index(max_objects))
            )
    else:
        ...
        return channel.ImageLabeled(
            extract_slice(L.pixels, 0, most_populated_closest_to_focus)
        )
    return L
...
def run_binarizer(
    ...
    if(args.gaussian > 0):
        logging.info(f"Gaussian low pass filter with sigma {args.gaussian}")
        img.gaussian(args.gaussian)
    ...
    logging.info((args.min_Z, img.axis_shape(img.axes[0])))
    z_size_range = (args.min_Z * img.axis_shape(img.axes[0]), np.inf)
    logging.info(f"filtering Z size: {z_size_range}")
    L.filter_size(img.axes[0], z_size_range)
